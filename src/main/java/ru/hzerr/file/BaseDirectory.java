...
public abstract class BaseDirectory implements IFSObject {
    ...
    abstract <T extends BaseDirectory> T createSubDirectory(String dirName) throws IOException;
    abstract <T extends BaseFile> T createSubFile(String fileName) throws IOException;
    abstract <T extends BaseDirectory> T getSubDirectory(String dirName);
    abstract <T extends BaseFile> T getSubFile(String fileName);

    abstract <T extends BaseFile> HStream<T> getFiles();
    abstract <T extends BaseDirectory> HStream<T> getDirectories();
    abstract <ID extends BaseDirectory, IF extends BaseFile>
    DoubleHStream<ID, IF> getFiles(boolean recursive) throws IOException;

    abstract boolean clean() throws IOException;

    abstract <T extends BaseDirectory> boolean deleteExcept(T... directories) throws IOException;
    abstract <T extends BaseFile> boolean deleteExcept(T... files) throws IOException;

    abstract <ID extends BaseDirectory, IF extends BaseFile>
    boolean deleteExcept(DoubleHStream<ID, IF> excludedFiles) throws IOException;
    abstract boolean delete(String dirOrFileName) throws IOException;

    abstract <T extends BaseDirectory>
